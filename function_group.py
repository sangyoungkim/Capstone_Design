...
get_df = get_stock_information("삼성전자",3)
...
# SVM으로 다음날 주식 가격 예측
stock_df_td = stock_df.tail(100) # 최근100일정도만 training
# print(stock_df_td)
# print(stock_df.info())

stock_df_train = stock_df.tail(len(stock_df_td)-1) # label 뺌
stock_df_close = stock_df_train['Close']
stock_df_close = stock_df_close.values

days=[]
for i in range(1,len(stock_df_train)+1):
    days.append([int(i+1)])
