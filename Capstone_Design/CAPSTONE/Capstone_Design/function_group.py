            ...
            if(len(stock[RT]) != 0):
                ...
                while(len(a)<int(news_num)):
                    ...
                    if(soup.select_one(".list_news .bx:nth-child("+str(num)+") .news_area").get_text().find(str(RT_stock)) != -1): 
                        ...
                        box_dic["사진"] = soup.select_one(".list_news .bx:nth-child("+str(num)+") a>img").attrs['src']
                        a.append(box_dic)
                    num = num + 1
                if(len(a) >= int(news_num)):
                    RT_arr.append(RT)
                    news_info.append(a)
                    box_dic = {}
                ...
                a=[]
                num = 1
        stock_arr.append(box_arr)
    RT_arr = list(set(RT_arr))
    stock_arr = list(filter(None,stock_arr))
...
#출처는 네이버 증권입니다.
    ...
    return df
a,b,c = keyword_stock_news_naver(RT_search_text())
print(a)
print("==================")
print(b)
print("==================")
